...
module.exports = class wLightBoxSDevice extends Homey.Device {
	...
	async pollDevice() 
	{
		while (this.polling && !this.pinging) {
			await util.sendGetCommand('/api/light/state',this.getSetting('address'))
			.then(result => {
				// On success - update Homey's device state
				this.setAvailable();
				let brightness = Math.round(parseInt(result.light.desiredColor,16)/255*100)/100;
				let onState = false;
				if(brightness>0)
				{
	...
	async pingDevice() 
	{
		while (!this.polling && this.pinging) {
			...
			.then(result => {
				if(result.device.type=='wLightBoxS' && result.device.id==this.getData().id)
				{
					this.setAvailable();
					this.polling = true;
					this.pinging = false;
